        ...
        try {
            if ($skills != []) {
                ...
                if (isset($_SESSION['id'])) {
                    $sql = "SELECT j.id, c.id as companyId, c.name, c.logo, j.title, j.location, j.type, j.minSalary, j.maxSalary FROM job j, company c WHERE (j.title LIKE '%$cleanedKeyword%' OR c.name LIKE '%$cleanedKeyword%') AND j.minSalary BETWEEN $cleanedMinSalary AND $cleanedMaxSalary AND j.maxSalary BETWEEN $cleanedMinSalary AND $cleanedMaxSalary AND j.location LIKE '%$cleanedLocation%' AND j.type LIKE '%$cleanedType%' AND j.id IN (SELECT jobId FROM jobskills WHERE skillId IN ($cleanedSkillsId)) AND j.id NOT IN (SELECT jobId FROM jobapplication WHERE userId = " . $_SESSION['id'] . ") AND j.blocked = 0 AND c.blocked = 0 AND c.blocked = 0 AND j.companyId = c.id ORDER BY createdAt DESC;";
                }
                else {
                    $sql = "SELECT j.id, c.id as companyId, c.name, c.logo, j.title, j.location, j.type, j.minSalary, j.maxSalary FROM job j, company c WHERE (j.title LIKE '%$cleanedKeyword%' OR c.name LIKE '%$cleanedKeyword%') AND j.minSalary BETWEEN $cleanedMinSalary AND $cleanedMaxSalary AND j.maxSalary BETWEEN $cleanedMinSalary AND $cleanedMaxSalary AND j.location LIKE '%$cleanedLocation%' AND j.type LIKE '%$cleanedType%' AND j.id IN (SELECT jobId FROM jobskills WHERE skillId IN ($cleanedSkillsId)) AND j.blocked = 0 AND c.blocked = 0 AND c.blocked = 0 AND j.companyId = c.id ORDER BY createdAt DESC;";
                }
            }
            else {
                if (isset($_SESSION['id'])) {
                    $userId = $_SESSION['id'];
                    $sql = "SELECT j.id, c.id as companyId, c.name, c.logo, j.title, j.location, j.type, j.minSalary, j.maxSalary FROM job j, company c WHERE (j.title LIKE '%$cleanedKeyword%' OR c.name LIKE '%$cleanedKeyword%') AND j.minSalary BETWEEN '$cleanedMinSalary' AND '$cleanedMaxSalary' AND j.maxSalary BETWEEN '$cleanedMinSalary' AND '$cleanedMaxSalary' AND j.location LIKE '%$cleanedLocation%' AND j.type LIKE '%$cleanedType%' AND j.id NOT IN (SELECT jobId FROM jobapplication WHERE userId = '$userId') AND j.blocked = 0 AND c.blocked = 0 AND c.blocked = 0 AND j.companyId = c.id ORDER BY createdAt DESC;";
                }
                else {
                    $sql = "SELECT j.id, c.id as companyId, c.name, c.logo, j.title, j.location, j.type, j.minSalary, j.maxSalary FROM job j, company c WHERE (j.title LIKE '%$cleanedKeyword%' OR c.name LIKE '%$cleanedKeyword%') AND j.minSalary BETWEEN '$cleanedMinSalary' AND '$cleanedMaxSalary' AND j.maxSalary BETWEEN '$cleanedMinSalary' AND '$cleanedMaxSalary' AND j.location LIKE '%$cleanedLocation%' AND j.type LIKE '%$cleanedType%' AND j.blocked = 0 AND c.blocked = 0 AND j.companyId = c.id ORDER BY createdAt DESC;;";
                }
            }
